...
from src.structures.species import Mythical

__all__ = ("MythicalCharacter",)


@dataclass(unsafe_hash=True, slots=True)
class MythicalCharacter(Character):
    def __post_init__(self):
        super(MythicalCharacter, self).__post_init__()
        self.sp_ability = None
        self.abilities = self.species.abilities

    @property
    def species(self) -> Mythical:
        return self.species
    ...
    async def upsert(self, connection: Connection):
        ...
        connection : Connection
            asyncpg connection
        """
        await self.standard_upsert(connection, kind="MYTHICAL")
