...
from src.structures.species import Mega

__all__ = ("MegaCharacter",)


@dataclass(unsafe_hash=True, slots=True)
class MegaCharacter(Character):
    def __post_init__(self):
        super(MegaCharacter, self).__post_init__()
        self.sp_ability = None
        self.abilities = self.species.abilities

    @property
    def species(self) -> Mega:
        return self.species
    ...
    async def upsert(self, connection: Connection):
        ...
        connection : Connection
            asyncpg connection
        """
        await self.standard_upsert(connection, kind="MEGA")
