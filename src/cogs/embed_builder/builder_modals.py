...
class EmbedModal(Modal):
    def __init__(self, embed: Embed) -> None:
        super().__init__(title="Embed's information", timeout=None)
        self.embed = embed
        self.reference = embed.copy()
        self.e_title = TextInput(
            label="Title",
            default=self.reference.title,
            max_length=256,
        )
        self.e_url = TextInput(
            label="URL",
            default=self.reference.url,
        )
        description = self.reference.description or ""
        self.e_description = TextInput(
            label="Description",
    ...
    async def on_submit(self, interaction: Interaction) -> None:
        ...
        self.reference.timestamp = parse(
            e_timestamp,
            settings=dict(
...
class EmbedModificationView(View):
    ...
    @button(
        label="Embed's Information".center(80, "\u2008"),
