...
from src.structures.character import Character, CharacterArg, doc_convert, oc_process
...
class Submission(commands.Cog):
    ...
    async def abilities_checker(self, ctx: Interaction, message: Message):
        ...
        if oc := self.ocs.get(message.id):
            ...
            abilities = [oc.sp_ability for oc in self.ocs.values() if oc.sp_ability]
            abilities.extend(Ability.all())
            abilities = [
                ab
                for ab, _, _ in process.extract(
                    text.title(),
                    choices=abilities,
                    processor=lambda x: getattr(x, "name", x),
                    score_cutoff=85,
            ...
            if not species.types:
                view = Complex(
                    ...
                    parser=lambda x: (
                        ...
                        f"Adds the typing {'/'.join(i.name for i in x)}",
                    ),
                    text_component=TextInput(
                        label="Fusion Typing",
    ...
    async def on_message_tupper(self, message: Message, member_id: int):
        ...
        if oc.location != channel.id:
            async with self.bot.database() as db:
                ...
                await db.execute(
                    "UPDATE CHARACTER SET LOCATION = $1 WHERE ID = $2",
                    channel.id,
                    oc.id,
                )
    ...
    async def load_profiles(self):
        ...
        elements = {oc.author: oc for oc in self.ocs.values() if oc.author}
        for author_id, oc in elements.items():
            self.oc_list[author_id] = oc.thread
            view = RPView(author_id, self.oc_list)
            self.bot.add_view(view=view, message_id=oc.thread)
