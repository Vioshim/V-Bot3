    ...
    async def find(
        ...
        if isinstance(mon, Species):
            ...
            if ab_text := "\n".join(f"• {ab.name}" for ab in species.abilities):
                embed.add_field(
                    name=f"Abilities (Max {species.max_amount_abilities})",
                    value=ab_text,
                )
            ...
            if isinstance(species, Fusion):
                if pronoun == Pronoun.She:
                    image1, image2 = (
                        species.mon1.female_image,
                        species.mon2.female_image,
                    )
                else:
                    image1, image2 = species.mon1.base_image, species.mon2.base_image
            elif pronoun == Pronoun.She:
                image1, image2 = species.female_image, species.female_image_shiny
            else:
                image1, image2 = species.base_image, species.base_image_shiny
