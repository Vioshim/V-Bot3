...
class MessagePaginator(Complex[Message]):
    ...
    @select(row=1, placeholder="Select the messages", custom_id="selector")
...
class MessageView(View):
    def __init__(
        ...
        super(MessageView, self).__init__(timeout=None)
        self.parser = parser
        if group_key:
            self.group_key = group_key
        else:
            self.group_key = lambda x: x.embeds[0].footer.text or ""
    ...
    def group_method(self, messages: set[Message]):
        messages = sorted(filter(lambda x: x.webhook_id and x.embeds, messages), key=self.group_key)
    ...
    def setup(self):
        ...
        for key, value in self.data.items():
            sct.add_option(label=key, value=key, description=f"{len(value)} messages.", emoji=self.emoji)
        if not sct.options:
    ...
    @select(placeholder="Messages by Category", row=3, custom_id="msg-filter")
    async def select_msg(self, ctx: Interaction, sct: Select):
        resp: InteractionResponse = ctx.response
        item = sct.values[0]
        if items := self.data.get(sct.values[0]):
            view = MessagePaginator(member=ctx.user, target=ctx, messages=items, parser=self.parser)
            embed = view.embed
